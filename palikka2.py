...
class Ball():
	def __init__(self, x, y, screen, width, height, speedx, speedy):
		self.x = x
		self.y = y
		self.screen = screen
		self.width = width
	...
	def update(self, dt):
		global playerShip, playerShip2, game_over
		if self.x + self.width >= playerShip.x and self.y < playerShip.y + playerShip.height and self.y > playerShip.y:
			self.speedx = -self.speedx

		if self.x + self.width <= playerShip2.x + PLAYER_WIDTH * 2 and self.y < playerShip2.y + playerShip2.height and self.y > playerShip2.y:
			self.speedx = -self.speedx
...
def startLevel():
	...
	ball = Ball(320, 240, screen, BALL_WIDTH, BALL_HEIGHT, BALL_SPEED_X, BALL_SPEED_Y)
	i = 0
	j = 0
